...
>回溯的核心思想：
>
>- 回溯的处理过程是一个穷举（枚举）的过程。枚举所有的解，找出其中满足期望的可行解。为了有规律的枚举所有可能的解，避免遗漏和重复，我们把问题求解的过程归纳为多阶段决策模型。每个阶段的决策会对应多个选择，从可选的选择列表中，任意选择一个，然后继续进行下一个阶段的决策。
...
>(1) 二维矩阵搜索或遍历
>
>使用深度优先搜索 DFS: 即给定一个m x n的二维矩阵时，dfs(i + 1, j) + dfs(i, j + 1)，先遍历深度 i -> 0 to m，再遍历宽度 j -> 0 to n
>
>(2) 最短路径 (广度优先搜索 BFS)
>
>(3) 连通分量 / 连通性
...
- [面试题08.10.颜色填充](https://leetcode-cn.com/problems/color-fill-lcci/)
...
> >
> > 状态转移方程：`dp[i][j] = max(dp[i-1][j], dp[i-1][j-weight[i]] + 1)` 即该状态是由上一个状态决策拿或不拿转移而来
>
...
- [72.编辑距离](https://leetcode-cn.com/problems/edit-distance/)
  ...
  # 状态: dp[m + 1][n + 1]
  # 状态转移方程:
  # 若当前word1[i - 1] == word2[j - 1]
  # dp[i][j] = min(dp[i][j - 1] + 1, dp[i - 1][j] + 1, dp[i - 1][j - 1])
  # 若当前word1[i - 1] != word2[j - 1]
  # dp[i][j] = min(dp[i][j - 1] + 1, dp[i - 1][j] + 1, dp[i - 1][j - 1] + 1)
  # p.s. 状态表格位置:    left                 up                left_up
  class Solution:
